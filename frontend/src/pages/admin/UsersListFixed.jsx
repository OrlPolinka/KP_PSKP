...
const UsersListFixed = () => {
  ...
  if (error) return (
    <div className="container">
      ...
      <button className="btn btn-primary" onClick={fetchUsers}>Повторить попытку</button>
    </div>
  );
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={6} style={{ textAlign: 'center', padding: '40px', color: 'rgba(255,255,255,0.3)' }}>
                    {users.length === 0 ? 'Пользователи не найдены в системе' : 'Пользователи не найдены по фильтру'}
                  </td>
                </tr>
              ) : filtered.map(user => {
