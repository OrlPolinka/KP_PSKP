...
const TrainersListFixed = () => {
  ...
  const fetchTrainers = async () => {
    try {
      ...
      setError('Не удалось загрузить тренеров. Проверьте подключение к серверу.');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (error) return (
    <div className="container">
      ...
      <button className="btn btn-primary" onClick={fetchTrainers}>Повторить попытку</button>
    </div>
  );
    ...
    <div className="container">
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          <Link to="/admin/trainers/public" className="btn btn-outline">
            👁️ Публичные страницы
          </Link>
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={5} style={{ textAlign: 'center', padding: '40px', color: 'rgba(255,255,255,0.3)' }}>
                    {trainers.length === 0 ? 'Тренеры не найдены в системе' : 'Тренеры не найдены по фильтру'}
                  </td>
                </tr>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.3)' }}>
                          <Link to={`/trainer/${trainer.id}`} style={{ color: 'var(--primary-light)', textDecoration: 'none' }}>
                            👁️ Публичная страница →
                          </Link>
                        </div>
                      </div>
                    </div>
...
export default TrainersListFixed;